...
(function () {
  ...
  function initLightbox() {
    var thumbs = document.querySelectorAll('[data-lightbox="media"]');
  ...
  /* ---- shopping cart / order (localStorage) ---- */
  function initCart() {
    ...
    var email = wrap.getAttribute("data-email") || "";
    var symbol = wrap.getAttribute("data-symbol") || "$";
    var pos = wrap.getAttribute("data-pos") || "before";
    var lRemove = wrap.getAttribute("data-l-remove") || "Remove";
    var lViaWa = wrap.getAttribute("data-l-via-wa") || "WhatsApp";
    var lViaEmail = wrap.getAttribute("data-l-via-email") || "Email";
    var mOrder = wrap.getAttribute("data-m-order") || "Order";
    var mQty = wrap.getAttribute("data-m-qty") || "Qty";
    var mUnit = wrap.getAttribute("data-m-unit") || "Unit";
    var mTotal = wrap.getAttribute("data-m-total") || "Total";
    ...
    function money(v) {
      ...
      return pos === "before" ? (symbol + s) : (s + " " + symbol);
    }
    function totalQty() {
    ...
    function addItem(data, qty) {
      ...
      for (var i = 0; i < cart.length; i++) {
        if (cart[i].slug === data.slug) { found = cart[i]; break; }
      }
      if (found) found.qty += qty;
      else cart.push({ slug: data.slug, ikey: data.ikey, price: data.price,
                       name: data.name, moq: data.moq, qty: qty });
      save(); render();
    }
    ...
    function removeItem(slug) {
      cart = cart.filter(function (it) { return it.slug !== slug; });
      save(); render();
    }
    ...
    function render() {
      ...
      countEl.style.display = totalQty() ? "grid" : "none";
      totalEl.textContent = money(totalSum());
      setSubmitEnabled(has);
      ...
      cart.forEach(function (it) {
        ...
        row.innerHTML =
          ...
          '<div class="ci-unit">' + mUnit + " " + money(it.price) + '</div>' +
          '<div class="ci-qty">' +
            '<button type="button" data-dec="' + esc(it.slug) + '">-</button>' +
            '<span class="q-num">' + it.qty + '</span>' +
            '<button type="button" data-inc="' + esc(it.slug) + '">+</button>' +
          '</div>' +
          '<button type="button" class="ci-remove" data-rm="' + esc(it.slug) + '">' + lRemove + '</button>';
    ...
    function buildOrder() {
      ...
      cart.forEach(function (it) {
        ...
        lines.push("- " + it.name + "  " + mQty + " x" + it.qty +
                   "  (" + mUnit + " " + money(it.price) + ")  = " + money(line));
      });
      lines.push("", mTotal + ": " + money(totalSum()));
      return lines.join("\n");
    }
    ...
    // Secondary "send via email" button next to the primary submit.
    var emailBtn = null;
    if (email) {
      emailBtn = document.createElement("button");
      emailBtn.type = "button";
      emailBtn.className = "btn btn-ghost cart-submit-email";
      emailBtn.textContent = lViaEmail;
      emailBtn.style.marginTop = "0";
      submitBtn.insertAdjacentElement("afterend", emailBtn);
    }
    ...
    document.addEventListener("click", function (e) {
      ...
      if (addBtn) {
        e.preventDefault();
        var qtyInput = document.getElementById("det-qty");
        var qty = 1;
        if (qtyInput && addBtn.classList.contains("ob-add")) {
          qty = parseInt(qtyInput.value, 10) || 1;
        }
        addItem({
          slug: addBtn.getAttribute("data-slug"),
          ikey: addBtn.getAttribute("data-ikey"),
          price: parseFloat(addBtn.getAttribute("data-price")) || 0,
          name: addBtn.getAttribute("data-name"),
          moq: addBtn.getAttribute("data-moq") || ""
        }, qty);
        var prev = addBtn.textContent;
        addBtn.classList.add("added");
        addBtn.textContent = "✓";
        setTimeout(function () { addBtn.classList.remove("added"); addBtn.textContent = prev; }, 1100);
        return;
      }
      ...
      // Open/close: match the FAB and its inner icon/label, the overlay,
      // and the close button (including any child nodes) reliably.
      if (fab && (e.target === fab || (fab.contains && fab.contains(e.target)))) {
        openDrawer(); return;
      }
      if (e.target === overlay || (e.target.closest && e.target.closest("#cart-close"))) {
        closeDrawer(); return;
      }
    });
    ...
    submitBtn.addEventListener("click", function () {
      if (!cart.length || !wa) return;
      window.open(wa + "?text=" + encodeURIComponent(buildOrder()), "_blank");
    });
    if (emailBtn) {
      emailBtn.addEventListener("click", function () {
        if (!cart.length) return;
        window.location.href = "mailto:" + email +
          "?subject=" + encodeURIComponent(mOrder) +
